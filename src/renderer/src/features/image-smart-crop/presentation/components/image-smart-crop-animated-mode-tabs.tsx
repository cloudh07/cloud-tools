...
import { ImageIcon, Images, type LucideIcon } from 'lucide-react'
import type { ReactElement } from 'react'

export type ImageSmartCropMode = 'single' | 'batch'

export type ImageSmartCropAnimatedModeTabsProps = {
  value: ImageSmartCropMode
  onValueChange: (mode: ImageSmartCropMode) => void
  disabled?: boolean
  className?: string
}

const MODES: readonly {
  id: ImageSmartCropMode
  label: string
  hint: string
  Icon: LucideIcon
}[] = [
  {
    id: 'single',
    label: 'Một ảnh',
    hint: 'Xem trước và lưu từng ảnh',
    Icon: ImageIcon
  },
  {
    id: 'batch',
    label: 'Hàng loạt',
    hint: 'Nhiều ảnh, cùng cài đặt',
    Icon: Images
  }
]

export function ImageSmartCropAnimatedModeTabs({
  value,
  onValueChange,
  disabled = false,
  className
}: ImageSmartCropAnimatedModeTabsProps): ReactElement {
  return (
    <div
      className={cn(
        ...
        {MODES.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                'relative flex min-h-14 flex-col items-center justify-center gap-0.5 rounded-lg px-3 py-2 text-center transition-colors sm:min-h-13 sm:flex-row sm:gap-2 sm:py-2.5 cursor-pointer',
