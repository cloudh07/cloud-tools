...
import { convertImageFormatWithSharp } from '@main/infrastructure/image/image-format-convert-sharp-service'
...
type QueueEntry = { req: StartImageFormatConvertBatchRequest }

export class ImageFormatConvertManager {
  ...
  private emit(event: ImageFormatConvertBatchEvent): void {
    const win = this.getWindow()
    if (!win || win.isDestroyed()) return
    win.webContents.send(IpcChannels.IMAGE_FORMAT_CONVERT_EVENT, event)
  }
  ...
  enqueue(request: StartImageFormatConvertBatchRequest): void {
  ...
  private async runBatch(req: StartImageFormatConvertBatchRequest): Promise<void> {
      ...
      for (let i = 0; i < req.items.length; i++) {
        ...
        try {
          ...
          this.emit({
            ...
            batchId: req.batchId,
            jobId: it.jobId,
            line: `[convert] ${it.inputPath} -> ${it.outputPath}`
          })
          ...
          await convertImageFormatWithSharp({
            inputPath: it.inputPath,
            outputPath: it.outputPath,
            target: req.outputFormat,
            options: req.options,
